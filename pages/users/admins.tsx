...
import { deleteUserFromAPI, getAllUsersFromAPI, iAPIUSer, updateUserfromAPI } from '../../api_calls/users.api';
...
const AdminsPage = () => {
    ...
    const handleAgreeToDeactivate = async (role: USER_ROLE) => {
        const id = userID;
        ...
        if (result === true) {
            ...
            timedAlert(alertState, active === 'Active' ? 'Disabled User!' : 'Enabled User!', 'success');
        } else {
            setShowDeletePrompt(false);
            setShowStatusPrompt(false);
        }
    ...
    const columns = [
        ...
        { name: 'username', label: 'Username', options: { filter: true, sort: true,setCellHeaderProps: () => ({ style: TableHeaderStyles }), } },
        { name: 'email', label: 'Email', options: { filter: true, sort: true,setCellHeaderProps: () => ({ style: TableHeaderStyles }), } },
        { name: 'role', label: 'Role', options: { filter: true, sort: true,setCellHeaderProps: () => ({ style: TableHeaderStyles }), } },
        {
            name: 'active',
            label: 'Status',
            options: {
        ...
        actions: (
            <Fragment>
                ...
                <AgreementModal
                    ...
                    onAgree={() => handleAgreeToDeactivate(user.role)}
                />
            </Fragment>
        ),
    }));
            ...
            <AdminOnly>
                <DashboardLayout>
                    {loading ? (
                        ...
                        <Fragment>
                            <Head>
                                <title>Dashboard | All Admins</title>
                            </Head>
                            <MUIDataTable
                                title="All Admins"
                                columns={columns}
