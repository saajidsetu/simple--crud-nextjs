...
export const createReceipt = async (req: Request, res: Response) => {
    try {
        const {  rate, v_number, date, check } = req.body;
        const user: iApiUser = req.body.api_user;

        const newReceipt = new Receipt({  rate, v_number, date, check });
...
export const updateReceipt = async (req: Request, res: Response) => {
    const { receiptId, rate, v_number, date, check } = req.body;
...
export const getAllReceipts = async (req: Request, res: Response) => {
    try {
        ...
        if (published === 'true') {
            ...
            const receipts = await Receipt.find({ publish: true }).sort({ date: 'asc' });
