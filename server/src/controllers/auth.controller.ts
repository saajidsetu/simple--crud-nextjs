...
export const getUserInfo = async (req: Request, res: Response) => {
    ...
    try {
        const userFound = await User.findById(user._id).select('_id name email username role ');
        if (!userFound) {
...
export const loginUser = async (req: Request, res: Response) => {
    ...
    try {
        const userFound = email ? await User.findOne({ email }) : await User.findOne({ username });
...
export const userForgotPassword = async (req: Request, res: Response) => {
    try {
        ...
        const userFound = await User.findOne({ email }).select('_id firstname lastname email username role ');
