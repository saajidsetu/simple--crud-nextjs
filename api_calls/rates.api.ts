...
export const getAllRatesFromAPI = async (): Promise<iAPIRate[]> => {
    try {
        ...
        const res = await fetch(`${API_BASE}/rates`, { headers: { 'skon-auth-token': token } });
...
export const deleteRateFromAPI = async (id: string): Promise<'success' | 'fail'> => {
    try {
        const token = <string>localStorage.getItem('skon-auth-token');
...
export const createRateFromAPI = async (data: iRate): Promise<GENERAL_RETURN_TYPE> => {
    try {
...
export const getSingleRateFromAPI = async (id: string): Promise<iAPIRate | null> => {
    try {
        const token = <string>localStorage.getItem('skon-auth-token');
        const res = await fetch(`${API_BASE}/rates/${id}`, { headers: { 'skon-auth-token': token } });
        const data = await res.json();
        return data;
    } catch (err: any) {
...
export const updateRatefromAPI = async (id: string, rate: iRate): Promise<boolean> => {
    try {
        const token = <string>localStorage.getItem('skon-auth-token');
		const formData = new FormData();
