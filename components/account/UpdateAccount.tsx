...
import { getSingleUserFromAPI, updateUserAccountfromAPI } from "../../api_calls/users.api";
...
import accountsValidator, { iSingleAccount } from "../validators/accounts.validator";
...
const UpdateAccount = () => {
    ...
    const [initialAccount, setInitialAccount] = useState(accountsValidator.initialSingleAccount);
	...
	useEffect(() => {
		...
		const getAccount = async () => {
			...
			if (data) {
				...
				setInitialAccount({ name: data.name, username: data.username, email: data.email, role: data.role });
				setLoading(false);
			}else {
				router.push('/')
			}
						...
						{(formProps: any) => (
							<Form>
								<Card sx={{ padding: '0.5rem 1rem', margin: '0.5rem 0', width: '600px' }}>
									<Typography variant="h6" sx={{ marginBottom: '2rem'}}>Update Account</Typography>
									<TextInput
										field_name="name"
										formProps={formProps}
										label="Name"
